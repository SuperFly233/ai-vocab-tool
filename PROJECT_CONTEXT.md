...
- Improved result highlighting. `highlightText()` now expands common English inflections and highlights the full word form instead of only the base stem; `/api/analyze` also asks the model to emit optional `highlightTerms` for senses and collocations so translations can highlight the expression that actually appears even when it differs from `shortestLabel`.
