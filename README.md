...
- 结果：支持排版视图和分区 JSON 视图；多词性词条会先按词性分组展示义项，每个词性组内单独编号，义项、核心义、搭配和易混词都有更清晰的层级。
- 追问：可针对当前词条继续向 AI 提问，回答会排版在结果下方并保存到对应历史记录；回答支持常见 Markdown 版式，包括标题、列表、引用块、代码块、分隔线、链接、删除线和行内强调。
