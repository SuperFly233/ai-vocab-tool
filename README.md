...
- 设置：可保存多组 API URL、API Key、Model 配置并切换当前使用项；外层只显示当前配置和下拉入口，新增、选择、编辑和删除收在配置菜单中，配置菜单会作为顶层浮层显示并避开移动端底部导航；新增和编辑配置会在弹窗里完成，也可查询模型列表和测试当前填写内容；模型 Prompt 专区可查看“输入、前端请求、System Prompt、模型 JSON、校验渲染”的实现路径，支持查看默认查询规则、复制、保存自定义覆盖版或恢复默认；同时可切换主题、首页布局、字段标签显示方式和字体风格，支持恢复默认。顶部布局适合快速查词；左右布局是宽屏工作台，左侧保留可拖拽长输入和单个侧重点，右侧用于持续阅读结果。
