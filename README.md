...
## 最新进展
- v0.9.47 优化结果高亮：英文会按常见词形变化包住完整单词，义项和搭配可通过 `highlightTerms` 指定例句/译文里实际对应的高亮词，减少译文措辞和 shortest label 不一致时的漏标。
...
- 结果：支持排版视图和分区 JSON 视图；会区分单词和词组/表达并写入 `meta.entryType`；主查询会使用流式接口边接收模型 JSON 边显示原始 JSON，并从已经生成的文本中实时提取词条头、义项、搭配、语感和易混块更新排版预览；模型结束后仍会执行完整 JSON 校验和必要修复，历史保存只使用最终校验后的结构化结果；核心义会单独突出显示，类型、词性和方向作为同一行辅助信息；多词性词条会先按词性分组展示义项，每个词性组内单独编号，义项、搭配和易混词都有更清晰的层级；例句、译文和语意会高亮当前词条、对应义标或 `highlightTerms`，英文会识别常见屈折变化并包住完整词形，方便对照；语言、方向、词性和语体会遵循设置里的字段标签显示偏好。
...
- 设置：可保存多组 API URL、API Key、Model 配置并切换当前使用项；外层只显示当前配置和下拉入口，新增、选择、编辑和删除收在配置菜单中，配置菜单会作为顶层浮层显示并避开移动端底部导航；新增和编辑配置会在弹窗里完成，也可查询模型列表和测试当前填写内容；同时可切换主题、首页布局、字段标签显示方式和字体风格，支持恢复默认。
