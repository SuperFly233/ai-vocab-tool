...
## 最新进展
- v0.10.2 修正历史详情编辑区顶部显示：恢复历史详情弹窗顶部标题、时间和操作按钮的原有展示方式；取消滚动后额外出现的压缩信息浮块，避免它在可视化编辑或 JSON 编辑时占据页面中部空间。
- v0.10.1 补齐可视化填写提示与历史搜索范围：可视化编辑改为字段级填写气泡，支持临时关闭和设置里常驻显示；历史搜索框右侧新增搜索范围，可限定只搜单词、释义、例句、搭配、辨析或其他项；编辑模式隐藏顶部学习信息块；API 配置菜单改为真正 fixed 浮层并用拖拽排序，Prompt 与 API 设置区域继续撑满右侧空间。
- v0.10.0 纠正历史排序并升级编辑体验：移除误加的创建/修改日期范围筛选，历史排序拆成“修改”和“创建”两个明确按钮；未编辑记录只显示一个裸时间，已编辑记录按设置显示创建、修改或全部；修复较长 `translationHighlights` 无法完整命中的问题；可视化编辑补齐例句/译文高亮、语义感受、近义/易混词，并支持条目排序；设置页大模块支持折叠，模型 Prompt 工作区撑满宽度，API 配置列表固定显示并支持上下调整顺序。
...
- `patch`：单个 bug、文案、样式或局部交互修复，例如 `0.10.1`。
- `minor`：成组体验或结构变化，例如历史交互、可视化编辑、设置架构这类跨模块更新，所以从 `0.9.x` 升到 `0.10.0`。
- `1.0.0`：核心路径稳定版。查询、同步、API 配置、历史编辑、移动端主要交互和部署说明都稳定后再发布，不把仍在频繁打磨的阶段提前叫 1.0。
...
- 历史记录：自动保存查询结果，整条记录可点击查看详情，支持排版 / 可视化编辑 / JSON / 编辑保存，也支持收藏、搜索、多选筛选，以及按修改时间或创建时间排序；列表会显示核心释义、单词/词组类型、词性、方向、语言摘要、自定义标签和按设置偏好选择的创建/修改时间，创建和修改相同时只显示一个时间，真正编辑过才显示明确的创建/修改信息；历史详情可维护 Tag 和个人 Note，Note 支持现有 Markdown 渲染并提供常用格式快捷按钮；设置页可汇总管理 Tag，支持筛选、重命名和批量移除；结果页会提示与当前词条互相包含的历史记录，方便在单词和词组之间跳转。
- 设置：可保存多组 API URL、API Key、Model 配置并切换当前使用项；外层只显示当前配置和下拉入口，新增、选择、编辑、删除和上下调整顺序收在配置菜单中，配置菜单会作为固定顶层浮层贴近当前配置条并避开移动端底部导航；新增和编辑配置会在弹窗里完成，也可查询模型列表和测试当前填写内容，有草稿时点击遮罩不会误关；模型 Prompt 专区可查看“输入、前端请求、System Prompt、模型 JSON、校验渲染”的实现路径，支持查看默认查询规则、复制、保存自定义覆盖版或恢复默认；模型接口、外观、数据和日志等设置模块支持折叠展开；同时可切换主题、首页布局、字段标签显示方式、字体风格和历史时间显示方式，支持恢复默认。顶部布局适合快速查词；左右布局是宽屏工作台，左侧保留可拖拽长输入和单个侧重点，右侧用于持续阅读结果。
