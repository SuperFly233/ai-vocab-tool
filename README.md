...
- v0.16.2 将云端认证改为应用第一启动任务，并让所有登录相关入口主动补启动认证；即使手机端历史或设置恢复异常，也不会再永久停在“云端正在初始化”。
- v0.16.1 修复手机端登录误报“Supabase 未配置”：Supabase SDK 改为随 Cloudflare 站点自托管，不再依赖 jsDelivr 缓存或访问状况；初始化提示也会准确区分加载失败与项目未配置。
- v0.16.0 重组设置页：桌面使用吸顶任务目录，手机使用横向分类条，所有模块默认展开并可定位/折叠；简洁 Snackbar 可选择顶部或底部且默认置顶。通知状态、返回、展开与版本箭头全部换成真正的 Lucide 图标，旋转和对齐不再依赖字符字体。
- v0.15.0 将右上角账号入口拆为轻量身份菜单与独立 `/account` 账号中心，登录、注册、同步和密码安全不再挤进弹窗；桌面导航只在中央保留首页、历史记录和收藏夹，设置与关于移动到右侧图标区。深色 Snackbar 修复对比度，品牌图标增加自适应底板，关于页改为全宽扁平信息流。
- v0.14.3 将查询队列和收藏夹目录统一为可持久化的长按拖拽排序，修复收藏夹顺序被记录数量覆盖及手机操作换行；启动时自动合并同词历史与相同版本，移动端顶部补回定稿品牌图标。Toast/Snackbar 固定在应用最顶层；历史详情三种视图共用草稿，遮罩、关闭和版本切换不会再静默丢失编辑；手机端列表标题与操作保持同一行。历史写入会复用归一化缓存，并跳过不可见列表渲染。
- v0.14.2 定稿双对话框与圆形放大镜品牌图标，统一站内入口、favicon 和移动端 manifest；Toast/Snackbar 提升到全局顶层，同时修复历史详情横向版本列表的拖拽方向判断。
- v0.14.1 移除顶部重复设置入口，修复桌面首页吸顶后语言方向与收藏夹消失的问题；API 配置、历史版本和可视化编辑区块统一改为带浮层、交换动画与边缘自动滚动的长按拖拽。
- v0.14.0 将产品品牌更新为 Lexi酱，移除顶部彩色条并统一品牌图标、favicon、主题滑块和全局控件；设置页默认折叠并精简 Prompt 工作区，收藏夹选择器改为紧凑多选弹窗，历史新增时间范围筛选并移除重复查看按钮，默认通知改为可滑动关闭的底部 Snackbar。
- v0.13.2 让受限浏览器中的本地存储读取安全降级，补齐 Supabase 认证网络抛错与初始化失败处理，并合并历史搜索的连续输入渲染，避免大历史库逐键重复扫描。
- v0.13.1 将浏览器图标资源从完整 Lucide 包缩减为 25 个实际使用图标，并加入遗漏检查；恢复出厂、主题、布局、离线模式、查询队列与草稿统一经过可回滚本地存储事务。
- v0.13.0 重构桌面/移动端应用外壳、圆润控件和本地图标体系，重新拉开主搜索与次级控件层级；同时新增可回滚的本地存储事务，避免历史、墓碑和收藏夹设置只成功一半。
- v0.12.2 修复恢复默认 Prompt 后旧自定义值可能再次同步回来的问题，并让缺少 id/时间字段的旧 API 配置与收藏夹采用确定性迁移，不再在每次读取时生成随机身份或当前时间。
- v0.12.1 为查询崩溃恢复增加完整请求凭证，同词不同方向、侧重点、收藏夹或重新生成目标不再被误判完成；直接打开收藏夹详情链接后的返回也会留在本站目录。
- v0.12.0 重构首页滚动工具栏、收藏夹层级和主题控件：手机滚动后默认只保留主搜索，方向/收藏夹/队列分层展开；收藏夹使用竖向目录进入详情，桌面为安静的主从列表；同时修复设置内容在相同更新时间下的设备顺序依赖。
- v0.11.25 让关于页更新记录真正按需加载：默认只渲染最近 6 个版本并展开最新一条，本次发布后更早 88 个版本可按需查看；手机初始页面高度从约 14052px 降至 2183px，桌面从约 13141px 降至 1617px。
- v0.11.24 修复手机历史详情头部的控件裁切和关闭键越界；收藏夹内容与拖拽顺序改为独立同步，另一台设备较新的重命名不会再被旧名称覆盖，并发新增仍会保留。
- v0.11.23 让 API 配置拖拽顺序真正参与云同步：配置内容与顺序独立合并，较新的拖拽不会被旧设备打回，并发新增和另一端较新的 Model 编辑仍会保留。
- v0.11.22 为收藏夹定义和 API 配置增加删除墓碑，解决旧设备把已删除条目重新同步回来的问题；并发新增仍保留，删除后的明确重建仍可恢复。新增纯数据测试、生产合并浏览器验证和 Cloudflare 资源契约。
- v0.11.21 修复字段时钟的重复确认边界：再次确认已收藏/已加入收藏夹会刷新时钟，编辑页保存“仍为空”的收藏夹状态也会形成权威空状态，不再被另一设备上的过期操作反压。
- v0.11.20 为收藏、收藏夹成员和旧标签增加独立更新时间，明确移除后不会再被旧设备副本复活；旧记录继续兼容并集规则。手机首页同时收回查询行右侧无效留白，主输入框更宽，紧凑态次级控件等高，并已在隔离 Edge 中验证贴顶与零横向溢出。
- v0.11.19 重组历史详情页头部：标题、模式切换、复制/导出和关闭键各自成组；关闭键固定右上，手机端模式栏可横滑且文件操作常驻。隔离 Edge 已验证桌面、手机均无横向溢出，首页等待态继续铺到底并居中。
- v0.11.18 修复多设备历史合并的顺序依赖：较新记录决定版本顺序，历史 id、标签、收藏夹、备注、追问和墓碑采用确定性规则；浏览器中正反/重复合并一致，100 组交换测试无失败。
- v0.11.17 用 Supabase 行版本替代周期性完整下载：稳定轮询只读取 6 个键的 `key + updated_at`；3000 条样本中响应约从 2.67MB 降至 439B，发现远端变化时仍自动执行完整合并。
- v0.11.16 为自动云轮询增加原始值快速比较：3000 条历史完全一致时平均约 0.14ms，不再执行平均约 160.8ms 的完整历史合并；真实远端新增仍会正常合并、恢复和回写。
- v0.11.15 将自动云同步改为按脏数据项增量上传并加入 180ms 合批；版本化快照保证旧请求不会清除并发产生的新修改，远端读取期间的新本机编辑也不会被覆盖。手机首页同时为右侧浏览器悬浮工具保留安全区，关键按钮保持完整可点。
- v0.11.14 优化大量历史下的切页性能：缓存历史/墓碑/设置解析结果，空筛选跳过全量字段扫描；3000 条压力样本中历史页桌面约从 137ms 降到 28ms、手机从 133ms 降到 19ms，并保留存储变化后的即时失效。
- v0.11.13 增加崩溃安全的本机查询任务队列：活动请求与排队项会跨刷新恢复，恢复前检查历史保存时间以避免重复调用模型，并加入中断、完成保护、顺序和去重专项测试。
- v0.11.12 增加历史删除墓碑，阻止离线删除的记录被旧云副本复活；同 id 追问按更新时间合并；历史详情关闭键强制固定在右上角，滚动后的首页搜索控件真正贴顶，并新增历史数据专项回归测试。
- v0.11.11 增加前端契约防回归检查：自动核对版本号、重复 DOM id、内联按钮函数、二级页面资源路径与 Vercel rewrite，并接入 `npm run check`。
- v0.11.10 补齐页面路由：历史记录、收藏夹、设置和关于页拥有独立 URL，刷新和浏览器前进/后退会恢复对应页面，Cloudflare Pages 与 Vercel 支持直接打开二级地址。
- v0.11.9 修正手机端主查询控件：顶部布局严格保持单行，精简占位文案，避免文字折行后被固定高度裁切；左右布局继续支持多行输入。
- v0.11.8 修正历史详情页和首页空状态的几何布局：关闭键脱离横向工具栏并固定在弹窗右上角；等待查询面板按真实视口边界动态铺底，与桌面侧栏或手机底栏对齐，等待文字在内容区严格居中。
- v0.11.7 优化移动端搜索和详情页操作：手机首页改为两行查询控件，滚动后搜索区真正固定到顶部；历史详情页的关闭按钮固定右上角，版本列表与重新生成按钮同排；详情页支持左缘右滑关闭，版本/API 配置/可视化编辑块支持长按拖动排序；版本去重不再强制按时间重排，避免手动顺序被打回。
- v0.11.6 修正首页空态铺底和搜索吸顶：无结果时结果面板重新铺到首屏底部；滚动后标题栏收起，搜索区固定到视口顶部，主搜索框、方向、收藏夹和查询按钮会真正贴顶可见。
- v0.11.5 收回首页巨型留白：修复顶栏布局下输入卡片被空工作区撑成大面板的问题；空结果态改为轻量高度；主搜索框更宽更高，方向、收藏夹和搜索按钮统一小一档。
- v0.11.4 修复首页吸顶与卡顿：顶栏布局下结果区回到页面滚动，搜索区下滑时继续固定；主搜索框更宽，方向和收藏夹控件收窄并统一高度；历史/收藏夹派生数据增加缓存，收藏夹详情分批渲染；首页查询草稿会本机保存以便刷新/崩溃后恢复；关于页更新记录折叠；`流石の～も` 这类单字符假名片段也能正确高亮。
- v0.11.3 修正收藏夹页面溢出与间距：收藏夹面板补齐内边距，标题和右侧操作不再贴边；桌面端左侧收藏夹列表收住横向溢出；页面和内部滚动条外观隐藏但保留滚动能力；首页收藏夹选择并入搜索横条，过多收藏夹会用 `+N` 收束；相同内容不会因为加入收藏夹或重复生成而制造重复版本。
- v0.11.2 收敛历史页收藏入口：历史列表不再显示旧星标收藏按钮，也不再保留隐藏的“收藏范围”状态；收藏相关操作统一通过收藏夹入口和“添加到收藏夹”弹窗完成。
- v0.11.1 完善收藏夹工作流：首页查询可预选收藏夹并随结果保存，收藏夹页可直接“查词加入此夹”；历史页移除旧“收藏”范围入口，历史卡片可通过弹窗多选加入收藏夹；收藏夹左栏支持新建、删除策略确认和拖拽排序；静态资源加版本参数，避免旧脚本缓存继续触发历史导入 `formatTime is not defined`。
- v0.11.0 新增收藏夹：侧边栏新增“收藏夹”入口，星标记录进入“个人收藏”，自定义收藏夹可新建、打开、重命名、移除和导出当前夹内 JSON；旧 Tag 会自动迁移为同名收藏夹，同一条历史记录可加入多个收藏夹，并预留 `parentId` 供后续做嵌套。
- v0.10.7 修复历史导入预检：读取 JSON 文件后不再因为时间范围显示调用不存在的 `formatTime` 而失败。
- v0.10.6 补齐历史文件导入并铺满设置页：导入历史支持点击选择 JSON 文件和拖拽文件，读取后自动预检；设置页桌面端改为左说明、右控件的宽屏布局，API、Prompt、导入区和标签管理会尽量铺满右侧空间。
- v0.10.5 新增历史 JSON 导入合并：设置页数据区可粘贴导出的历史 JSON，先预检新增、重叠、可更新和重复数量，再合并导入并同步云端；支持普通数组、`raw` 字符串和 Supabase `value.raw` 格式。
- v0.10.4 修复历史记录 120 条上限：移除保存和云同步合并里的历史截断，历史总数会按真实存储数量显示；列表仍分批渲染，避免大量记录一次性铺开导致卡顿。
- v0.10.3 修复可视化提示常驻开关：关闭“常驻显示”后再次进入可视化编辑页会恢复字段聚焦气泡；云同步合并不再把旧的开启状态永久保留下来，`"false"` 这类字符串值也不会被误判为开启。
...
- 历史记录：自动保存查询结果，整条记录可点击查看详情，支持排版 / 可视化编辑 / JSON / 编辑保存，也支持收藏、搜索、多选筛选，以及按修改时间或创建时间排序；列表会显示核心释义、单词/词组类型、词性、方向、语言摘要、自定义标签和按设置偏好选择的创建/修改时间，创建和修改相同时只显示一个时间，真正编辑过才显示明确的创建/修改信息；历史详情可维护 Tag 和个人 Note，Note 支持现有 Markdown 渲染并提供常用格式快捷按钮；设置页可汇总管理 Tag，支持筛选、重命名和批量移除；数据区支持导出历史和粘贴 JSON 预检合并导入，便于从旧设备或备份里找回不重叠记录；结果页会提示与当前词条互相包含的历史记录，方便在单词和词组之间跳转。
